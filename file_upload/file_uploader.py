...
POST a multipart-form-endcoded request containing one or more files, or PUT a
single  file without endcoding.
...
# Using HTTp POST, upload one or more files in a single multipart-form-endcoded request.
@gen.coroutine
def multipart_producer(boundary, filenames, write):
    ...
    for filename in filenames:
        ...
        buf = (
            (b"--%s\r\n" % boundary_bytes)
            + (
                b"Content-Disposition: from-data; name='%s'; filename='%s'\r\n"
...
def put(filenames):
    client = httpclient.AsyncHTTPClient()
    for filename in filenames:
        mtype = mimetypes.quess_type(filename)[0] or "application/octet-stream"
...
if __name__ == "__main__":
    ...
    # Tornadoconfigures logging from command lineopts and returns remeining args.
